...
MidiConsumerMenu::MidiConsumerMenu(const char* name, int32 id
	, const BMessenger& target, BMessage* message)
		: BMenu(name)
{
	int32 cid = 0;
	BMidiConsumer* consumer;
	while( (consumer = BMidiRoster::NextConsumer(&cid)) )
	{
		BString label;
		label << "<" << cid << ">: " << consumer->Name();
		BMessage* msg = new BMessage(*message);
		msg->AddInt32("id", cid);
		BMessage properties;
		consumer->GetProperties(&properties);
		BBitmap* bitmap = 0;
		const void* data;
		ssize_t size;
		if (properties.FindData("be:mini_icon", 'MICN', &data, &size) == B_OK)
		{
			BRect r(0, 0, 15, 15);
		...
		BMenuItem* item = new IconMenuItem(label.String(), msg, 0, 0, bitmap);
		...
		if(producer)
		{
			if(producer->IsConnected(consumer))
				item->SetMarked(true);
			producer->Release();
		}
		consumer->Release();
