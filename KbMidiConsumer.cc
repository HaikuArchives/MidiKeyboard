...
KbMidiConsumer::KbMidiConsumer(const char* name)
	: BMidiLocalConsumer(name)
{
}

void KbMidiConsumer::NoteOff(uchar channel, uchar note
	,	uchar velocity, bigtime_t time)
{
	midi_engine->note_off(channel, note, velocity);
}

void KbMidiConsumer::NoteOn(uchar channel, uchar note,
	uchar velocity, bigtime_t time)
{
	midi_engine->note_on(channel, note, velocity, time);
}

void KbMidiConsumer::Data(uchar *data, size_t length, bool atomic, bigtime_t time)
{
	midi_engine->data(time);
	BMidiLocalConsumer::Data(data, length, atomic, time);
}

void KbMidiConsumer::ControlChange(uchar channel
		, uchar controlNumber, uchar controlValue, bigtime_t time)
{
	midi_engine->control_change(channel, controlNumber, controlValue);
}
