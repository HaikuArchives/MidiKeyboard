...
MidiProducerMenu::MidiProducerMenu(const char* name, int32  id
	, const BMessenger& target, BMessage* message)
		: BMenu(name)
{
	int32 pid = 0;
	BMidiProducer* producer;
	while( (producer = BMidiRoster::NextProducer(&pid)) )
	{
		BString label;
		label << "<" << pid << ">: " << producer->Name();
		BMessage* msg = new BMessage(*message);
		msg->AddInt32("id", pid);

		BMessage properties;
		producer->GetProperties(&properties);
		BBitmap* bitmap = 0;
		const void* data;
		ssize_t size;
